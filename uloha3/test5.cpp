...
{
  public:
    CPatchStr() : m_segments(nullptr), m_segmentCount(0), m_capacity(1) {}
...
void CPatchStr::ensureCapacity(size_t required) {
    if (required <= m_capacity)
        return;

    size_t newCapacity = std::max(required, m_capacity * 2);
    StringList * newSegments = new StringList[newCapacity];
    for (size_t i = 0; i < m_segmentCount; ++i) {
...
char * CPatchStr::toStr() const {
    ...
    for (size_t i = 0; i < m_segmentCount; ++i) {
        std::memcpy(result + currentPos, m_segments[i].ptr->str.get() + m_segments[i].ofs, m_segments[i].len);
...
CPatchStr & CPatchStr::append(const CPatchStr & src) {
    ensureCapacity(m_segmentCount + src.m_segmentCount);

    // Copy segments from the source into the existing array
    for (size_t i = 0; i < src.m_segmentCount; ++i) {
        m_segments[m_segmentCount + i] = src.m_segments[i];
    }
