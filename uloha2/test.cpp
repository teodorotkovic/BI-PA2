...
class CBigInt
{
  private:
    ...
    CBigInt addDigit(const CBigInt& val1, const CBigInt& val2) {
        ...
        this->m_digits = resultDigits;
        this->m_negative = resultNegative;
  ...
  public:
    ...
    explicit CBigInt(const char* value) {
    ...
    CBigInt& operator=(const char* other) {
        *this = CBigInt(other);
        return *this;
    }
    CBigInt& operator=(const int other) {
        *this = CBigInt(other);
        return *this;
    }
    ...
    CBigInt& operator+=(const CBigInt& other) {
        *this = addDigit(*this, other);
        return *this;
    }
    CBigInt& operator+=(const char* other) {
        return *this += CBigInt(other);
    }
    CBigInt& operator+=(const int other) {
        return *this += CBigInt(other);
    }
    ...
    CBigInt& operator*=(const CBigInt& other) {
        ...
        std::string product = multiplyPositiveStrings(m_digits, other.m_digits);
        m_negative = m_negative != other.m_negative; // Update sign based on multiplication rule
        m_digits = product;
        return *this;
    }
    CBigInt& operator*=(const char* other) {
    ...
    // Overloads for comparison with int
    ...
    // Overloads for comparison with const char*
...
CBigInt operator+(const int lhs, const CBigInt& rhs) {
    CBigInt temp(lhs);
    return temp + rhs;
}
CBigInt operator+(const char* lhs, const CBigInt& rhs) {
    CBigInt temp(lhs);
    return temp + rhs;
}
...
bool operator<(const int& lhs, const CBigInt& rhs) {
    CBigInt temp(lhs);
    return temp < rhs;
}
bool operator<=(const int& lhs, const CBigInt& rhs) {
...
bool operator<(const char* lhs, const CBigInt& rhs) {
    CBigInt temp(lhs);
    return temp < rhs;
}
bool operator<=(const char* lhs, const CBigInt& rhs) {
...
void testExceptions() {
    try {
        CBigInt a("invalid123");
        assert(false); // Should not reach here
    } catch (const std::invalid_argument& e) {
        assert(true); // Correctly thrown
    }
}
void fuzz() {
    CBigInt a{};
